...
public class StudentMyfileUploadServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		File classfile= new File(excel,student.getSclass());
		if (!classfile.exists()) {
			classfile.mkdir();
		}
		...
		try {
			...
			for (FileItem item : list) {
				...
				if(!item.isFormField()){
					...
					try {
						item.write(new File(classfile,fileName));
						bean.setCode("10000");
						bean.setObj(classfile+"\\"+fileName);
						bean.setMsg(fileName);
